...
console.log(false || true); // => true: This will return true
console.log(false || 'John'); // => John: This will return true because a non empty string is a Truthy value.
console.log(false || -1); // => -1: This will return true because any number different of 0 is a Truthy value.
console.log(false || 1 || 2); // => 1: This returns 1 because is the first Truthy operand.
...
console.log(userColor || defaultColor); // => red: This basically means that if we have a value for the user color we use that, if we don't we use the default color.

userColor = undefined;
console.log(userColor || defaultColor); // => blue: Here the user did not pick a color, the variable user is set to undefined, so we will use the default color.
